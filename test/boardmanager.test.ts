...
suite("Arduino: Board Manager.", () => {
    ...
    test("should be able to load board packages", function(done) {
        ...
        boardManager.loadPackages(false).then(() => {
            ...
            assert.equal(platforms[0].rootBoardPath, Path.join(Resources.mockedIDEPackagePath, "arduino", "avr"), "Should be able to index root board path for installed boards");
